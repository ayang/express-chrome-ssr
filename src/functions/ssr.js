 ...
 *     a new browser instance is launched.
 */
export async function ssr_langs(url, browserWSEndpoint, screenSize) {
	...
	try {
		...
		try {
			...
			for (let lang of ["cn", "hk"]) {

				await page.evaluate(() => {
					window.reduxStore.dispatch({type: "CHANGE_URL_LANG", payload: lang});
				});
